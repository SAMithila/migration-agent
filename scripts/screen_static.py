...
cutoff = datetime.now(timezone.utc).timestamp() - (30 * 30 * 24 * 3600)  # loosened: 30 months
allowed_licenses = {"MIT", "Apache-2.0", "BSD-3-Clause", "BSD-2-Clause", "ISC", "MPL-2.0", "Unlicense", "0BSD", None}
# None included: many repos have a real permissive LICENSE file but no SPDX id set by GitHub
...
for r in repos:
    ...
    if size_kb > 80000:     # loosened from 50000
...
with open("corpus/passed_static.json", "w") as f:
    json.dump(passed, f, indent=2)
with open("corpus/rejected_static.json", "w") as f:
